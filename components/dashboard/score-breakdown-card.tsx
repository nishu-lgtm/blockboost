...
const IMPACT_STYLE: Record<DriverImpact, { bar: string; text: string; bg: string; label: string }> = {
  critical: { bar: "bg-red-500",    text: "text-red-700",    bg: "bg-red-50 border-red-200",       label: "Critical" },
  high:     { bar: "bg-amber-500",  text: "text-amber-700",  bg: "bg-amber-50 border-amber-200",   label: "High impact" },
  medium:   { bar: "bg-slate-400",  text: "text-slate-600",  bg: "bg-slate-50 border-slate-200",   label: "Medium" },
  low:      { bar: "bg-emerald-500",text: "text-emerald-700",bg: "bg-emerald-50 border-emerald-200",label: "Low impact" },
};
        ...
        {drivers.map((d) => {
          ...
          return (
            <Link
              key={d.label}
              href={href}
              className={`block p-3 rounded-lg border ${style.bg} hover:shadow-sm transition-shadow group`}
            >
              <div className="flex items-center gap-3">
                ...
                <div className="w-16 shrink-0">
                  ...
                  <div className="h-1.5 bg-white/60 rounded-full overflow-hidden mt-0.5">
                    {d.hasData && (
                      <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <Badge variant="outline" className={`text-[10px] ${style.text} ${style.bg}`}>
                      {style.label}
                    </Badge>
                  </div>
                  <p className="text-xs text-slate-500 line-clamp-1">{d.why}</p>
                  <p className={`text-[11px] mt-0.5 ${d.hasData ? "text-slate-600" : "text-slate-400 italic"}`}>
                    {d.detail}
                  </p>
                </div>
